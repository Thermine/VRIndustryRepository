    ...
    public class HVRTeleporter : MonoBehaviour
    {
        ...
        public virtual void Teleport(Vector3 position, Vector3 direction)
        {
            ...
            if (Player)
            {
                ...
                if (LeftHand) leftRot = Quaternion.Inverse(Player.transform.rotation) * LeftHand.transform.rotation;
                if (RightHand) rightRot = Quaternion.Inverse(Player.transform.rotation) * RightHand.transform.rotation;

                Player.FaceDirection(direction);

                if (LeftHand) LeftHand.transform.rotation = Player.transform.rotation * leftRot;
                if (RightHand) RightHand.transform.rotation = Player.transform.rotation * rightRot;
            }
        }
